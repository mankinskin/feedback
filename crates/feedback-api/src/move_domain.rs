...
//!
//! Canonical entity folders (see [`crate::canonical`]) participate in a
//! move. The feedback store has no
...
use crate::canonical::{
    CanonicalFeedbackStore, FEEDBACK_ENTITY_FILE_NAME, FEEDBACK_ENTITY_SUBDIR,
...
impl CanonicalFeedbackStore {
    ...
    pub fn plan_move_preflight(
        ...
        move_kernel::plan_move(&domain, entity_id, target_workspace_root).map_err(from_move_error)
    }
    ...
    pub fn execute_move_with_journal(&self, plan: &MovePlan) -> Result<MoveOutcome, String> {
        let domain = FeedbackMoveDomain::new(self);
        move_kernel::execute_move(&domain, plan).map_err(from_move_error)
    }

    /// Resume an interrupted single-entity move from its journal id.
    pub fn resume_move_with_journal(&self, journal_id: Uuid) -> Result<MoveOutcome, String> {
        let domain = FeedbackMoveDomain::new(self);
        move_kernel::resume_move(&domain, journal_id).map_err(from_move_error)
    }

    /// Roll back a single-entity move from its journal id.
    pub fn rollback_move_with_journal(&self, journal_id: Uuid) -> Result<MoveOutcome, String> {
        let domain = FeedbackMoveDomain::new(self);
    ...
    pub fn execute_move_set(&self, plan: &MoveSetPlan) -> Result<MoveSetOutcome, String> {
        let domain = FeedbackMoveDomain::new(self);
    ...
    pub fn resume_move_set(&self, journal_id: Uuid) -> Result<MoveSetOutcome, String> {
        let existing =
            load_move_set_journal(self.store_root(), journal_id).map_err(from_move_error)?;
        if matches!(
            existing.phase,
    ...
    pub fn rollback_move_set(&self, journal_id: Uuid) -> Result<MoveSetOutcome, String> {
        let domain = FeedbackMoveDomain::new(self);
